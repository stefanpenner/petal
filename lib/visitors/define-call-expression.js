...
DefineCallExpression.prototype.enter = function(node) {
  if (node.callee.name === 'define') {
    ...
    switch (args.length)
    {
      case 1:
        ...
        if (type === 'FunctionExpression') {
          ...
          // detect simplified CJS
          if (firstArg.params.length === 3 &&
              firstArg.params[0].name === 'require' &&
              firstArg.params[1].name === 'exports' &&
              firstArg.params[2].name === 'module') {
            imports = ['require', 'exports', 'module'];
          }
    ...
    var moduleIndex = imports.indexOf('module');
    var exportsName, moduleName;

    if (exportsIndex > -1) {
      exportsName = callback.params[exportsIndex].name;
    }
    if (moduleIndex > -1) {
      moduleName = callback.params[moduleIndex].name;
    }
    ...
    } else if (exportsName || moduleName) {
      exports = exportsFromInput(exportsName, moduleName, body);
